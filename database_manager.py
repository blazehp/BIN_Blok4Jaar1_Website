...
class _DatabaseManager:
    """
    Class containing a handful of utility functions to ease
    ...
    def insert(self):
        ...
        try:
            ...
            value_placeholders = ",".join(
                ["%s" for _ in self.data.keys()]
            )
            values = tuple(self.data.values())

            if None in values:
                warnings.warn(
                    "Inserting NONE values may cause errors"
                )
    ...
    def update(self, position, pvalue):
        ...
        **NOTE**: If a column's value is `None`, it will be omitted from
        the query.
        ...
        for column, value in self.data.items():
            ...
            if isinstance(value, (int, float)):
                query = (
                    f"UPDATE {self.table} SET {column} = {value} "
                    f"WHERE {position} = {pvalue}"
                )
            else:
                query = (
                    f"UPDATE {self.table} SET {column} = '{value}' "
